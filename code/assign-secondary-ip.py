...
def get_instance_id():
    ...
    try:
        ...
        if code == 200:
            ...
            return (instanceid,region,privateIp)
...
## This function reads the details of the worker node and retruns the network interface id when IP address is matched
def get_instance_eni(ec2_client,instance_id,privateIp):
    eni_id=None
    ...
    for r in response['Reservations']:
      for i in r['Instances']:
        for j in i["NetworkInterfaces"]:
            if j["PrivateIpAddress"] == privateIp :
                eni_id= j["NetworkInterfaceId"]
                break
    return  eni_id
def main():    
    instance_id = None
    ...
    while (1) :
        ...
        try:
            ...
            if not instance_id :
                ...
                region = data[1]
                privateIp = data[2]
                tprint ("Got InstanceId: " + instance_id + " region: " + region + " privateIp: "+privateIp)  
                ec2_client = boto3.client('ec2', region_name=region)
                eni_id = get_instance_eni(ec2_client,instance_id,privateIp)
            ...
            # in most of the cases, its single IP, however code handles if there are more than 1 IP passed
            if ipaddress : 
                ip = IPNetwork(ipaddress)
                cidr = str(ip.cidr)
                if  netaddr.valid_ipv4(str(ip.ip)):                  
                    assign_ip_to_nic([str(ip.ip)],eni_id,ec2_client)   
                    tprint ("Finished secondary IPV4 assignment: "+str(ip.ip)) 
                else :
                    ##ipv6 doesnt have the reassign flag, so release it before from old ENI and then ssign to new ENI
                    release_ipv6([str(ip.ip)],ec2_client)
                    assign_ip6_to_nic([str(ip.ip)],eni_id,ec2_client)  
                    tprint ("Finished secondary IPV6 assignment: "+str(ip.ip))                
